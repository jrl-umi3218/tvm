...
namespace tvm::manifold::operators
{
  ...
  public:
    ...
    {
      return LG::log(LG::compose(Y, LG::inverse(X)));
    }
    ...
    static auto jacobians(const LHS& X, const RHS& Y, FromSide = {}, ToSide = {})
    {
      ...
      // t = log(Z) with Z = Y o X^-1
      ret_t t = value(X, Y);
      if constexpr (internal::isLocal<FromSide>())
      {
        // Dt/^{X}DX, Dt/^{Y}DY
        const auto& invJr = LG::invRightJacobian(t);
        return std::make_pair(matrix_t(-invJr * LG::adjoint(X).matrix()), matrix_t(invJr * LG::adjoint(X).matrix()));
      }
      else
      {
        // Dt/^{E}DX, Dt/^{E}DY
        const auto& [invJr, invJl] = LG::invJacobians(t);
        return std::make_pair(matrix_t(-invJr), invJl);
      }
    }
  };
}
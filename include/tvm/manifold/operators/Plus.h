...
namespace tvm::manifold::operators
{
  ...
  public:
    ...
    {
      // Z = X o Y
      if constexpr (internal::isLocal<ToSide>())
      {
        if constexpr (internal::isLocal<FromSide>())
        {
          // ^{Z}DZ/^{X}DX, ^{Z}DZ/^{Y}DY
          int dim = LG::dynamicDim(Y);
          return std::make_pair(LG::adjoint(Y).inverse().matrix(), LG::jacobian_t::Identity(dim, dim));
        }
        else
        {
          // ^{Z}DZ/^{E}DX, ^{Z}DZ/^{E}DY
      ...
      else
      {
        ...
        else
        {
          ...
          return std::make_pair(LG::jacobian_t::Identity(dim, dim), LG::adjoint(X).matrix());
        }
      }
    }
  };

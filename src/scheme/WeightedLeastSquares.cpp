    ...
    void WeightedLeastSquares::build()
    {
      ...
      for (auto c : constraints)
      {
        ...
        if (p == 0)
        {
          ...
          internal::AssignmentTarget target(r, {memory_, &memory_->C}, { memory_, &memory_->l }, { memory_, &memory_->u }, constraint::RHS::AS_GIVEN, x_.size());
          assignments_.emplace_back(internal::Assignment(c.constraint, c.requirements, target, x_));
          m0 += c.constraint->size();
        }
        else
        {
          RangePtr r = std::make_shared<Range>(m1, c.constraint->size()); //FIXME: for now we do not keep a pointer on the range nor the target.
          internal::AssignmentTarget target(r, { memory_, &memory_->A }, { memory_, &memory_->b }, constraint::Type::EQUAL, constraint::RHS::AS_GIVEN);
          assignments_.emplace_back(internal::Assignment(c.constraint, c.requirements, target, x_, std::pow(scalarizationWeight_,p-1)));

...
#define DOCTEST_VERSION_STR \
  DOCTEST_TOSTR(DOCTEST_VERSION_MAJOR) "." DOCTEST_TOSTR(DOCTEST_VERSION_MINOR) "." DOCTEST_TOSTR(DOCTEST_VERSION_PATCH)
...
DOCTEST_INTERFACE bool decomp_assert(assertType::Enum at, const char * file, int line, const char * expr, Result result);
...
#    define DOCTEST_ASSERT_IMPLEMENT_1(assert_type, ...) [&] { DOCTEST_ASSERT_IMPLEMENT_2(assert_type, __VA_ARGS__); }()
...
bool decomp_assert(assertType::Enum at, const char * file, int line, const char * expr, Result result)
{
...
struct XmlReporter : public IReporter
{
  ...
  void report_query(const QueryData & in) override
  {
    ...
    else if(opt.count || opt.list_test_cases)
    {
      ...
      xml.scopedElement("OverallResultsTestCases").writeAttribute("unskipped", in.run_stats->numTestCasesPassingFilters);
    }
...
#    endif // DOCTEST_PLATFORM_WINDOWS
    ...
    {
      xml.startElement("testcase").writeAttribute("classname", testCase.classname).writeAttribute("name", testCase.name);
      if(opt.no_time_in_output == false)

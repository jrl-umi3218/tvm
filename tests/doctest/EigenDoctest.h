...
#define EIGEN_APPROX_PREFIX \
    template <typename T> friend typename std::enable_if<std::is_constructible<Ref<const MatrixXd>, T>::value, bool>::type
    ...
    friend doctest::String toString(const Approx& in)
    {
      return "Approx(" + doctest::toString(in.value_) + ")\n";
    }

  private:
    template<typename Derived>
    bool eq(const DenseBase<Derived>& other) const { return value_.isApprox(other, epsilon_); }
